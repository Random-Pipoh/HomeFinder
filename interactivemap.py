...
with tab2:
    ...
    # Optional: Show images of the sites below the map
    st.subheader("Photos of All Candidate Sites")
    for idx, row in data.iterrows():
        site_name = row['site_name'].strip()
        image_url = row['image_url']  # Ensure this column contains the correct image URLs
        if image_url:
            st.image(image_url, caption=site_name, use_container_width=True)  # Replaced use_column_width
        else:
            st.info(f"No image available for {site_name}.")
...
with tab3:
    ...
    with st.expander("About this AI Analysis"):
        st.markdown("""
        - The recommendations provided here are generated using <b>OpenAI's GPT model</b>.
        - They are based on proximity metrics and sentiment scores in the uploaded dataset.
        - Please note these insights are <b>not absolute facts</b>—they are generated based on patterns in the data and may reflect inherent biases.
        - Use them as <b>a guide</b>, not a final decision-making tool.
        """, unsafe_allow_html=True)

    st.markdown("<b>Customize Weighting</b> (optional)", unsafe_allow_html=True)
